...
class NeuralNetwork:
    ...
    def _single_backprop(self,
        ...
        # Get the correct activation backprop
        if activation_curr == "sigmoid":
            act_back = self._sigmoid_backprop(dA_curr, Z_curr)
        elif activation_curr == "relu":
            act_back = self._relu_backprop(dA_curr, Z_curr)

        # Compute partial derivatives
        dA_prev = np.dot(act_back, W_curr)
        dW_curr = np.dot(act_back.T, A_prev)
        db_curr = np.sum(act_back, axis = 0).reshape(b_curr.shape)
    ...
    def backprop(self, y: ArrayLike, y_hat: ArrayLike, cache: Dict[str, ArrayLike]):
        ...
        # Compute the first backprop on the loss function
        ...
        for idx, layer in reversed(list(enumerate(self.arch))):
            ...
            # Get weights and biases at current layer
            layer_idx = idx + 1
            w = self._param_dict[f"W{layer_idx}"]
            b = self._param_dict[f"b{layer_idx}"]

            # Run backprop using info from the cache and layer
            ...
            # Store results in gradient dictionary and set up next iteration
            grad_dict[f"dW{layer_idx}"] = dW_curr
            grad_dict[f"db{layer_idx}"] = db_curr
            dA_curr = dA_prev

        # Return final gradient dictionary
        return grad_dict
    ...
    def _update_params(self, grad_dict: Dict[str, ArrayLike]):
        ...
        # Go through architecture and do lr-scaled subtractions
        for idx, _ in enumerate(self.arch):
            layer_idx = idx + 1
